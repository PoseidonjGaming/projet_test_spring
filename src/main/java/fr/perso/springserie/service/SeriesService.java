...
public class SeriesService extends BaseService<Series, SeriesDTO> implements ISeriesService {
    ...
    private final ISaisonService saisonService;

    @Autowired
    public SeriesService(ISeriesRepo repo, IFileService fileService, ISaisonService saisonService) {
    ...
    public void saveWithFile(MultipartFile file, String series)  {
        ...
        if (file != null && !file.isEmpty()) {
            ...
            seriesObj.setAffiche(file.getOriginalFilename());
        }
        save(seriesObj);
    ...
    public SeriesDTO toDTO(Series entity) {
        ...
        dto.setSaisonsIds(saisonService.getBySeriesId(dto.getId()).stream().map(BaseDTO::getId).toList());
        return dto;
    }
}
